...
public class LRecyclerView extends RecyclerView {
    ...
    public void setOnRefreshListener(OnRefreshListener listener) {
        ...
        Log.e("LRecyclerView", "setOnRefreshListener-----listener=" + listener +
                ", mRefreshing=" + mRefreshing);
        mRefreshing = false;//解决在刷新列表时马上切换界面，会出现列表一直为空的情况。
    }
    ...
    public void refresh() {
        if (mRefreshHeader.getVisibleHeight() > 0 || mRefreshing) {// if RefreshHeader is Refreshing, return
            Log.e("LRecyclerView", "refresh-----getVisibleHeight=" + mRefreshHeader.getVisibleHeight() +
                    ", mRefreshing=" + mRefreshing);
            return;
        }
